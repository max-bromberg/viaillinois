...
- `RSOs.name`, one name per organization, with no aliases.
...
**Name.** The booking's name contains an organization's name as a whole word, compared
without case or accents. Organizations with short or common names are the risk here,
so a name shorter than four characters matches only as an exact whole word, and a match
from this signal alone never reaches the confidence the event overlap signal does.

**Customer.** Tableau's `customer` field compared against organization names in the
same way. It is stronger than the booking name where it exists, because it names who
...
3. **Organization aliases.** Many organizations are known by a short form that is not
   their name in VIA. Aliases would make the name signal much stronger, but they have
   to be entered by the organization rather than invented by VIA, which is a separate
   small piece of work.
